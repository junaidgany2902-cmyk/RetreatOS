...
# Custom CSS for the Luxury Editor UI
st.markdown("""
    <style>
...
with col_prev:
    ...
    <div style='background-color: {current_bg}; color: {current_text}; padding: 50px; border: 1px solid #eee; min-height: 800px; font-family: serif;'>
        ...
        <h1 style='font-size: 58px; margin: 0; line-height: 0.9;'>{retreat_name.upper()}</h1>
        ...
        <p style='font-size: 16px; font-weight: 300; line-height: 1.8; font-family: sans-serif;'>{vision}</p>
        
        <div style='display: flex; gap: 10px; margin-top: 30px;'>
            {f"<div style='width: 100px; height: 100px; background: #333; display: flex; align-items: center; justify-content: center; font-size: 10px; color: #666;'>IMAGE 1</div>" if gallery_images else ""}
            {f"<div style='width: 100px; height: 100px; background: #333; display: flex; align-items: center; justify-content: center; font-size: 10px; color: #666;'>IMAGE 2</div>" if len(gallery_images) > 1 else ""}
        </div>
        
        <div style='margin-top: 50px; padding: 20px; border: 1px solid {accent_color}; font-family: sans-serif;'>
        ...
        {f"<div style='margin-top: 20px; background: #2d5a27; color: white; padding: 10px; text-align: center; font-size: 11px; font-family: sans-serif;'>🛡 RESILIENCE VERIFIED: 100% SOLAR & WATER BACKUP</div>" if resilience_check else ""}
        
        <p style='margin-top: 40px; font-size: 10px; color: #888; text-align: center; font-family: sans-serif;'>CURATED BY {host_name.upper()}</p>
    </div>
    """
    ...
    if st.button("✨ Export High-End PDF", use_container_width=True):
        ...
        st.download_button("📥 Download Final Lookbook", data=bytes(pdf.output()), file_name=f"{retreat_name}_Lookbook.pdf", mime="application/pdf", use_container_width=True)

st.divider()
st.caption("RetreatOS v1.7.1 | Luxury B2B Edition")
